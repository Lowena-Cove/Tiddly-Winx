...
error_handler() {
    ...
    script="""
    set alertTitle to \"Kaijinix - Updater error\"
    set alertMessage to \"An error occurred during Kaijinix update (updater.sh:$lineno)\n\nPlease download the update manually from our website if the problem persists.\"
    ...
    if the button_pressed is \"Open Download Page\" then
        open location \"https://Kaijinix.org/download\"
    end if
    """
