...
if [ "$VERSION" == "1.1.0" ];
then
  RELEASE_TAR_FILE_NAME=sdl2-Kaijinix-headless-$CONFIGURATION-$VERSION+$SOURCE_REVISION_ID-macos_universal.tar
else
  RELEASE_TAR_FILE_NAME=sdl2-Kaijinix-headless-$VERSION-macos_universal.tar
fi

ARM64_OUTPUT="$TEMP_DIRECTORY/publish_arm64"
X64_OUTPUT="$TEMP_DIRECTORY/publish_x64"
UNIVERSAL_OUTPUT="$OUTPUT_DIRECTORY/publish"
EXECUTABLE_SUB_PATH=Kaijinix.Headless.SDL2
...
dotnet restore
dotnet build -c "$CONFIGURATION" src/Kaijinix.Headless.SDL2
dotnet publish -c "$CONFIGURATION" -r osx-arm64 -o "$TEMP_DIRECTORY/publish_arm64" "${DOTNET_COMMON_ARGS[@]}" src/Kaijinix.Headless.SDL2
dotnet publish -c "$CONFIGURATION" -r osx-x64 -o "$TEMP_DIRECTORY/publish_x64" "${DOTNET_COMMON_ARGS[@]}" src/Kaijinix.Headless.SDL2
...
tar --exclude "publish/Kaijinix.Headless.SDL2" -cvf "$RELEASE_TAR_FILE_NAME" publish 1> /dev/null
python3 "$BASE_DIR/distribution/misc/add_tar_exec.py" "$RELEASE_TAR_FILE_NAME" "publish/Kaijinix.Headless.SDL2" "publish/Kaijinix.Headless.SDL2"
