...
using static Kaijinix.Ava.UI.Helpers.Win32NativeInterop;
using AntiAliasing = Kaijinix.Common.Configuration.AntiAliasing;
using InputManager = Kaijinix.Input.HLE.InputManager;
using IRenderer = Kaijinix.Graphics.GAL.IRenderer;
using Key = Kaijinix.Input.Key;
using MouseButton = Kaijinix.Input.MouseButton;
using ScalingFilter = Kaijinix.Common.Configuration.ScalingFilter;
using Size = Avalonia.Size;
using Switch = Kaijinix.HLE.Switch;

namespace Kaijinix.Ava
{
                ...
                Task.Run(() =>
                {
                    lock (_lockObject)
                    {
                        ...
                        string directory = AppDataManager.Mode switch
                        {
                            ...
                            _ => Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyPictures), "Kaijinix"),
                        };
                        ...
                        if (SetupValidator.CanFixStartApplication(ContentManager, ApplicationPath, userError, out firmwareVersion))
                        {
                            ...
                            if (userError == UserError.NoFirmware)
                            {
                                ...
                                await ContentDialogHelper.CreateInfoDialog(
                                    ...
                                    "",
                                    LocaleManager.Instance[LocaleKeys.KaijinixInfo]);
                            }
                        }
                        else
                        {
