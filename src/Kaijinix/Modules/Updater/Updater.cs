    ...
    internal static class Updater
    {
        ...
        public static async Task BeginParse(Window mainWindow, bool showVersionUpToDate)
        {
            ...
            try
            {
                ...
                {
                    if (asset.Name.StartsWith("Kaijinix") && asset.Name.EndsWith(_platformExt))
                    {
            ...
            {
                Logger.Error?.Print(LogClass.Application, "Failed to convert the received Kaijinix version from Github!");
            ...
            await Dispatcher.UIThread.InvokeAsync(async () =>
            {
                ...
                var shouldUpdate = await ContentDialogHelper.CreateChoiceDialog(
                    LocaleManager.Instance[LocaleKeys.KaijinixUpdater],
                    LocaleManager.Instance[LocaleKeys.KaijinixUpdaterMessage],
                    $"{Program.Version} -> {newVersion}");

                if (shouldUpdate)
                {
                    await UpdateKaijinix(mainWindow, _buildUrl);
                }
                else
                {
                    _running = false;
        ...
        private static async Task UpdateKaijinix(Window parent, string downloadUrl)
        {
            ...
            {
                Header = LocaleManager.Instance[LocaleKeys.KaijinixUpdater],
            ...
            if (_updateSuccessful)
            {
                ...
                if (!OperatingSystem.IsMacOS())
                {
                    shouldRestart = await ContentDialogHelper.CreateChoiceDialog(LocaleManager.Instance[LocaleKeys.KaijinixUpdater],
                ...
                if (shouldRestart)
                {
                    ...
                    if (OperatingSystem.IsMacOS())
                    {
                        ...
                        string newBundlePath = Path.Combine(_updateDir, "Kaijinix.app");
                    ...
                    else
                    {
                        ...
                        if (ryuName.StartsWith("Kaijinix.Ava"))
                        {
                        ...
                        {
                            ryuName = OperatingSystem.IsWindows() ? "Kaijinix.exe" : "Kaijinix";
                        }
...
#pragma warning restore SYSLIB0014
        ...
        public static void CleanupUpdate()
        {
            ...
            // Migration: Delete old Kaijinix binary.
            // TODO: Remove this in a future update.
            if (!OperatingSystem.IsMacOS())
            {
                string[] oldRyuFiles = Directory.GetFiles(_homeDir, "Kaijinix.Ava*", SearchOption.TopDirectoryOnly);
                // Assume we are running the new one if the process path is not available.
                // This helps to prevent an infinite loop of restarts.
                string currentRyuName = Path.GetFileName(Environment.ProcessPath) ?? (OperatingSystem.IsWindows() ? "Kaijinix.exe" : "Kaijinix");

                string newRyuName = Path.Combine(_homeDir, currentRyuName.Replace(".Ava", ""));
                if (!currentRyuName.Contains("Kaijinix.Ava"))
                {
