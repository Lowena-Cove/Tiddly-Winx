...
using Kaijinix.Ava.UI.Views.User;
using LibHac;
using LibHac.Common;
using LibHac.Fs;
...
using Models_UserProfile = Kaijinix.Ava.UI.Models.UserProfile;
using UserId = Kaijinix.HLE.HOS.Services.Account.Acc.UserId;
using UserProfile = Kaijinix.Ava.UI.Models.UserProfile;

namespace Kaijinix.Ava.UI.Controls
{
    public partial class NavigationDialogHost : UserControl
    {
        ...
        public void LoadProfiles()
        {
            ...
            {
                ViewModel.Profiles.Add(new Models_UserProfile(profile, this));
            }
            ...
            while (true)
            {
                ...
                for (int i = 0; i < readCount; i++)
                {
                    ...
                    if (ViewModel.Profiles.Cast<Models_UserProfile>().FirstOrDefault(x => x.UserId == id) == null)
                    {
                        lostAccounts.Add(id);
                    }
                }
            ...
            {
                ViewModel.LostProfiles.Add(new Models_UserProfile(new HLE.HOS.Services.Account.Acc.UserProfile(account, "", null), this));
            }

            ViewModel.Profiles.Add(new BaseModel());
        }

        public async void DeleteUser(Models_UserProfile userProfile)
        {
            ...
            if (userProfile.UserId == lastUserId)
            {
                ...
                var profile = ViewModel.Profiles.Cast<Models_UserProfile>().FirstOrDefault(x => x.UserId != lastUserId);
        ...
        public void AddUser()
        {
            Navigate(typeof(UserEditorView), (this, (Models_UserProfile)null, true));
        }

        public void EditUser(Models_UserProfile userProfile)
        {
