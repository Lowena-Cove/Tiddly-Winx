...
using Kaijinix.HLE.Exceptions;
using Kaijinix.HLE.HOS.Ipc;
using Kaijinix.HLE.HOS.Kernel.Memory;
using Kaijinix.HLE.HOS.Kernel.Threading;
using Kaijinix.HLE.HOS.Services.Am.AppletAE.Storage;
using Kaijinix.HLE.HOS.Services.Am.AppletOE.ApplicationProxyService.ApplicationProxy.Types;
using Kaijinix.HLE.HOS.Services.Sdb.Pdm.QueryService;
using Kaijinix.HLE.HOS.SystemState;
using Kaijinix.Horizon.Common;
using System;
using System.Numerics;
using System.Threading;
using AccountUid = Kaijinix.HLE.HOS.Services.Account.Acc.UserId;
using ApplicationId = LibHac.Ncm.ApplicationId;

namespace Kaijinix.HLE.HOS.Services.Am.AppletOE.ApplicationProxyService.ApplicationProxy
{
    class IApplicationFunctions : IpcService
    {
        ...
        public ResultCode PrepareForJit(ServiceCtx context)
        {
            if (Interlocked.Exchange(ref _jitLoaded, 1) == 0)
            {
                ...
                {
                    throw new InvalidSystemResourceException("JIT (010000000000003B) system title not found! The JIT will not work, provide the system archive to fix this error. (See https://github.com/Kaijinix/Kaijinix#requirements for more information)");
                }
