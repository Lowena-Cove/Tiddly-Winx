...
using Kaijinix.Audio.Backends.Dummy;
using Kaijinix.Audio.Backends.OpenAL;
using Kaijinix.Audio.Backends.SDL2;
using Kaijinix.Audio.Backends.SoundIo;
...
using Kaijinix.Common.SystemInterop;
using Kaijinix.Cpu;
using Kaijinix.Graphics.GAL;
using Kaijinix.Graphics.GAL.Multithreading;
using Kaijinix.HLE.FileSystem;
using Kaijinix.HLE.HOS;
using Kaijinix.HLE.HOS.Services.Account.Acc;
using Kaijinix.HLE.HOS.SystemState;
using Kaijinix.Input.GTK3;
using Kaijinix.Input.HLE;
using Kaijinix.Input.SDL2;
using Kaijinix.Modules;
using Kaijinix.UI.App.Common;
using Kaijinix.UI.Applet;
using Kaijinix.UI.Common;
using Kaijinix.UI.Common.Configuration;
using Kaijinix.UI.Common.Helper;
using Kaijinix.UI.Helper;
using Kaijinix.UI.Widgets;
using Kaijinix.UI.Windows;
...
using ShaderCacheLoadingState = Kaijinix.Graphics.Gpu.Shader.ShaderCacheState;

namespace Kaijinix.UI
{
...
#pragma warning restore CS0649, IDE0044, CS0169, IDE0051
        ...
        public MainWindow() : this(new Builder("Kaijinix.Gtk3.UI.MainWindow.glade")) { }
        ...
        private MainWindow(Builder builder) : base(builder.GetRawOwnedObject("_mainWin"))
        {
            ...
            Icon = new Gdk.Pixbuf(Assembly.GetAssembly(typeof(ConfigurationState)), "Kaijinix.UI.Common.Resources.Logo_Kaijinix.png");
            Title = $"Kaijinix {Program.Version}";
            ...
            VirtualFileSystem.FixExtraData(_libHacHorizonManager.KaijinixClient);

            _contentManager = new ContentManager(_virtualFileSystem);
            _accountManager = new AccountManager(_libHacHorizonManager.KaijinixClient, CommandLineState.Profile);
        ...
        private bool LoadApplication(string path, ulong applicationId, bool isFirmwareTitle)
        {
            ...
            if (!SetupValidator.CanStartApplication(_contentManager, path, out UserError userError))
            {
                if (SetupValidator.CanFixStartApplication(_contentManager, path, userError, out firmwareVersion))
                {
                    ...
                    message = $"No installed firmware was found but Kaijinix was able to install firmware {firmwareVersion.VersionString} from the provided game.\nThe emulator will now start.";
            ...
            if (File.Exists(path))
            {
                switch (System.IO.Path.GetExtension(path).ToLowerInvariant())
                {
                    ...
                    default:
                        ...
                        {
                            Logger.Error?.Print(LogClass.Application, "The specified file is not supported by Kaijinix.");
        ...
        private void SwitchToGameTable()
        {
            ...
            Window.Title = $"Kaijinix {Program.Version}";
        ...
        private void Row_Clicked(object sender, ButtonReleaseEventArgs args)
        {
            ...
            _ = new GameTableContextMenu(this, _virtualFileSystem, _accountManager, _libHacHorizonManager.KaijinixClient, application);
        }
